...
$c_W \in [7556/65536, 1]$ (with $7556/65536 \approx 0.115$), and full
...
results --- early density estimates by Crandall~\cite{Crandall1978} and
Korec~\cite{Korec1994}, and most recently the breakthrough of
Tao~\cite{Tao2022}, showing that almost all trajectories attain almost
bounded values --- the conjecture itself remains open. We refer the reader
...
which equals $(1, 0)$ only for $v = 0$, contradicting $v \ge 1$; at $J = 1$ the
update rule gives $d_1 = (3 \cdot (-2^{-v}) + 2^{-v} - 1)/2^{\vi^{(0)}} = -(1 + 2^{1-v})/2^{\vi^{(0)}} \ne 0$
for $v \ge 1$, contradicting $d_J = 0$. Hence $J \ge 2$, so the case
...
symmetry of Proposition~\ref{prop:lift-symmetry} gives, per parent at $L - 1$:
