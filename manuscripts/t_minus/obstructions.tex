...
\subjclass[2020]{11B83, 37P05, 37B10, 68R15, 11A07}
...
of $\bigcup_L \Obs_L$ turns out to be all of $\{0,1\}^*$.
...
The shift index is bounded: since $\delta_J = \Vk^{(J)} - \Vi^{(J)} - v$ with
$0 \le \Vk^{(J)} \le L$ and $0 \le \Vi^{(J)} \le L - v$, we have
$|2a + v| = |\delta_J + v| = |\Vk^{(J)} - \Vi^{(J)}| \le \max(\Vk^{(J)}, \Vi^{(J)}) \le L$,
hence $|a| \le L/2$.
...
$\Obs_L \leftrightarrow \Obs_L^{+}$. The remaining claims --- lift,
positive density, full factor complexity --- transfer mechanically: by
Lemma~\ref{lem:involution-step} the involution conjugates each
$\Tminus$ basic step into the corresponding $\Tplus$ basic step, so the
case analyses of Sections~\ref{sec:lift}, \ref{sec:density},
and~\ref{sec:fc} apply verbatim. \qedhere
\end{proof}
