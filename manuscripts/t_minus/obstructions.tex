...
For an odd integer $r$ with $r \equiv 3 \pmod 4$ and $\vtwo(r-1) = 1$, we
...
(Definition~\ref{def:obstruction}).
...
finite invariant of the parallel reduction. (For $L_0 < 6$, $\Obs_{L_0}$ is
empty and the theorem is vacuous; the substantive content begins at
$L_0 = 6$.) The lift is explicit: for each $r_0$, the terminal data of the
reduction at $r_+$ or $r_-$ is determined by a closed-form algebraic
identity.
...
\]
satisfy $\Delta_K^{(0)} = 2^{L_0}$, $\Delta_I^{(0)} = 2^{L_0 - 1}$. We claim
that as long as $\Vk^{(j)} < L_0$ and $\Vi^{(j)} < L_0 - 1$, the two
...
$L_0 - \Vk^{(J)} < L_0 + 1 - \Vk^{(J)}$. The K-step at $(r_-, L_0+1)$ at
index $J$ is therefore permitted, and one more step follows. The update is
precisely the EE-extra-step from Theorem~\ref{thm:lift-plus}:
...
condition at level $L_0+1$. Termination occurs at index $J$ with terminal
data unchanged, so $r_- \in \Obs_{L_0+1}^{G_a}$. \qedhere
\end{proof}
...
Corollary~\ref{cor:entropy}. A non-trivial sofic shift is the image of an
SFT under a one-block factor map and inherits the entropy constraint.
Primitive substitution shifts have factor complexity in one of the
polynomial classes
$\Theta(1), \Theta(n), \Theta(n \log \log n), \Theta(n \log n), \Theta(n^2)$
(Pansiot~\cite{Pansiot1984}; refined by Devyatov~\cite{Devyatov2015}),
giving entropy zero --- but our $p_W(n) = 2^n$ has positive entropy. \qedhere
\end{proof}
