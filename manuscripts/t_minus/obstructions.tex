...
with $r \equiv 3 \pmod 4$ and $\vtwo(r-1) = 1$, write $m := (r-1)/2$. We run
...
\label{def:obstruction-intro}
An odd integer $r$ with $r \equiv 3 \pmod 4$ and $\vtwo(r-1) = 1$ is an
\emph{obstruction residue at level $L$} if $\Xend(r, L) = 1$. We write
$\Obs_L \subseteq \{1, \ldots, 2^L\}$ for the set of all such residues.
\end{definition}
...
Let $L_0 \ge 2$ and $r_0 \in \Obs_{L_0}$. Then both $r_+ := r_0$ and
...
finite invariant of the parallel reduction. The lift is explicit: for each
$r_0$, the terminal data of the reduction at $r_+$ or $r_-$ is determined by
a closed-form algebraic identity.
...
The shift-$a$ normal form is, to the best of our knowledge, new. The
...
$L \ge 1$, and consider a pair $(a, b)$ with $a$ a positive odd integer
and $b$ a power of two, interpreted as a residue $a$ modulo $b$. A single
$\Tminus$-step on such a pair produces
\[
(a', b') = \left(\frac{3a - 1}{2^{v_a}},\; \frac{3b}{2^{v_a}}\right), \qquad v_a := \vtwo(3a - 1),
\]
provided $v_a < \vtwo(b)$. If $v_a \ge \vtwo(b)$, the modulus has been
exhausted and the step is undefined; we say the reduction has \emph{terminated}.

Since $3$ is odd, $\vtwo(3b) = \vtwo(b)$, so $\vtwo(b') = \vtwo(b) - v_a$:
the modulus strictly decreases at each step, and the reduction always
terminates after finitely many steps.

\subsection{The two-track setup}

Fix $r$ a positive odd integer with $r \equiv 3 \pmod 4$ and $\vtwo(r-1) = 1$.
We postpone the general case $\vtwo(r-1) \ge 2$ to a remark at the end of
this section. Write $m := (r - 1)/2$; the conditions on $r$ ensure $m$ is
odd.

Fix a level $L \ge 2$. We run two $\Tminus$-reductions in parallel:
\begin{itemize}
\item the \emph{K-track}, starting from $(a_K^{(0)}, b_K^{(0)}) := (r, 2^L)$,
\item the \emph{I-track}, starting from $(a_I^{(0)}, b_I^{(0)}) := (m, 2^{L-1})$.
\end{itemize}
The initial relation $m = (r-1)/2$ writes as $a_I^{(0)} = \tfrac{1}{2} a_K^{(0)} - \tfrac{1}{2}$.
...
\emph{obstruction residue at level $L$} if $\Xend(r, L) = 1$. We write
\[
\Obs_L := \{\, r \in \{1, \ldots, 2^L\} : r \text{ is an obstruction residue at level } L \,\}.
\]
\end{definition}
...
\delta_J := \Vk^{(J)} - \Vi^{(J)} - 1
\]
is an \emph{even} integer (possibly negative), where
$\Vk^{(j)} := \sum_{i < j} \vk^{(i)}$ and analogously for $\Vi$.
\end{lemma}

\begin{proof}
Iterating the update rule from Lemma~\ref{lem:affine} with $c_0 = 1/2$:
\[
c_j = \tfrac{1}{2} \cdot \prod_{i < j} 2^{\vk^{(i)} - \vi^{(i)}} = 2^{\Vk^{(j)} - \Vi^{(j)} - 1} = 2^{\delta_j}.
\]
...
The shift index is bounded: $|2a + 1| \le \max(\Vk^{(J)}, \Vi^{(J)}) \le L$,
so $|a| \le L/2$.
...
condition $v_a \ge \vtwo(b)$ at the same index $J$.
\end{corollary}

\begin{proof}
From the shift-$a$ normal form, $a_I^{(J)} = 4^a a_K^{(J)} + (1-4^a)/3$, so
$A_I^{(J)} = 4^a A_K^{(J)}$ and hence $\vi^{(J)} = \vk^{(J)} + 2a$.
Combining with $\vtwo(b_I^{(J)}) = (L-1) - \Vi^{(J)}$ and
$\vtwo(b_K^{(J)}) = L - \Vk^{(J)}$, and using $\Vk - \Vi = 2a + 1$:
\[
\vi^{(J)} \ge \vtwo(b_I^{(J)}) \iff \vk^{(J)} + 2a \ge L - 1 - \Vi^{(J)} \iff \vk^{(J)} \ge L - \Vk^{(J)} \iff \vk^{(J)} \ge \vtwo(b_K^{(J)}). \qedhere
\]
\end{proof}
...
For $r$ with $v := \vtwo(r-1) \ge 2$, the I-track starts at $(m, 2^{L-v})$
with $m = (r-1)/2^v$, and the initial coefficients are $c_0 = 2^{-v}$,
$d_0 = -2^{-v}$. The parity lemma then asserts that $\Vk^{(J)} - \Vi^{(J)} - v$
is even, and the shift-$a$ normal form is unchanged. The analysis is
identical with $v$ replacing $1$ throughout. We carry $v = 1$ for concreteness;
...
$(m_0, 2^{L_0})$ where $m_0 = (r_0 - 1)/2$. These differ from the
...
\]
satisfy $\Delta_K^{(0)} = 2^{L_0}$, $\Delta_I^{(0)} = 2^{L_0 - 1}$. As long
as the $v$-values coincide between the two reductions (which is the case
while $\Vk^{(j)} < L_0$ and $\Vi^{(j)} < L_0 - 1$),
\[
\Delta_K^{(j)} = 2^{L_0 - \Vk^{(j)}} \cdot 3^j, \qquad \Delta_I^{(j)} = 2^{L_0 - 1 - \Vi^{(j)}} \cdot 3^j.
\]

\emph{Stop type SS at $L_0$.} At index $J$ of $(r_+, L_0+1)$,
$\vk^{(J)} > L_0 - \Vk^{(J)}$, hence $\vtwo(\Delta_K^{(J)}) = L_0 - \Vk^{(J)} < \vk^{(J)}$.
By $\vtwo(A + B) = \min(\vtwo A, \vtwo B)$ when $\vtwo A \ne \vtwo B$, the
K-step at $(r_-, L_0+1)$ at index $J$ produces
$v_{K,-}^{(J)} = L_0 - \Vk^{(J)} < L_0 + 1 - \Vk^{(J)}$. The step is permitted,
and one more step follows. The update is precisely the EE-extra-step from
Theorem~\ref{thm:lift-plus}: $(c_{J+1}, d_{J+1}) = (1, 0)$, so
$r_- \in \Obs_{L_0+1}^{G_0}$.

\emph{Stop type EE at $L_0$.} Here $\vk^{(J)} = L_0 - \Vk^{(J)} = \vtwo(\Delta_K^{(J)})$,
so $\vtwo(A_{K,-}^{(J)}) \ge \vk^{(J)} + 1 = L_0 + 1 - \Vk^{(J)}$ by the carry
identity. Termination occurs at index $J$ with terminal data unchanged, so
$r_- \in \Obs_{L_0+1}^{G_a}$.\qedhere
\end{proof}
...
\[
\Fac(\Obs) := \{\, u \in \{0,1\}^* : u \text{ appears as a contiguous substring of some } w_r,\; r \in \Obs_L,\; L \ge 1 \,\}.
\]
...
any anchor $r_0 \in \Obs_6 = \{19, 27, 59\}$. Define inductively
\[
...
has entropy strictly less than $\log 2$, contradicting
$h(\Fac(\Obs)) = \log 2$. A non-trivial sofic shift is the image of an SFT
under a one-block factor map and inherits the entropy constraint. Primitive
substitution shifts have factor complexity in one of the polynomial classes
$\Theta(1), \Theta(n), \Theta(n \log \log n), \Theta(n \log n), \Theta(n^2)$
(Pansiot~\cite{Pansiot1984}; refined by Devyatov~\cite{Devyatov2015}), giving
entropy zero --- but our $p_W(n) = 2^n$ has positive entropy. \qedhere
\end{proof}
...
\begin{proof}
By Theorem~\ref{thm:lift-intro} every $r \in \Obs_L$ has a unique smallest
$L_0 \ge 6$ with $r \bmod 2^{L_0} \in \Atom_{L_0}$, and $r$ is determined by
this atomic anchor together with the bits $\{u_{L_0}, \ldots, u_{L-1}\}$
encoding the lift choices. Each atomic anchor at level $L_0$ generates
$2^{L - L_0}$ obstructions at level $L$. \qedhere
\end{proof}
...
\begin{proof}
The basic $\Tplus$-step on $\overline{r}$ produces $v_+(\overline{r}) = \vtwo(3\overline{r} + 1) = \vtwo(-(3r - 1)) = \vtwo(3r - 1) = v_-(r)$,
so the affine update transfers without change. The terminal $X$-value is
the same. \qedhere
\end{proof}
...
The map $r \mapsto \overline{r}$ restricts to a bijection
$\Obs_L \leftrightarrow \Obs_L^+$. Theorems~\ref{thm:lift-intro},~\ref{thm:density-intro},
...
\subsection{Connection to Wirsching's framework}

The Lagarias intertwining of Section~\ref{sec:tplus} transfers our results
to $\Tplus$, but Wirsching's framework~\cite{Wirsching1998} approaches the
related predecessor-density problem from a Markov-operator perspective.
A unified treatment --- for instance, expressing $c_W$ in terms of
Wirsching's invariant density $\varphi$ --- appears not to be in the
literature.
...
collected in the directory \texttt{code/}
alongside the manuscript source. The directory contains the verification
scripts for each rigorous result (lift theorem, atom decomposition,
ATOMAR-$G_0$, full factor complexity, simultaneous stop) together with a
...
residues $r$ with $r \equiv 3 \pmod 4$ and $\vtwo(r-1) = 1$, runs the
parallel reduction, and records the terminal data. The set $\Obs_L$ is
read off as the preimage of $\{(c, d) : 3d + c = 1\}$.

We have run this enumeration through $L = 18$, producing the following
counts:
...
The data is consistent with Theorems~\ref{thm:lift-intro},~\ref{thm:no-G0-atom}
(strict lift balance), and~\ref{thm:series} (series convergence).
