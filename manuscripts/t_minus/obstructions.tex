...
anchor at some level $L_0 \in \{6, 7, \ldots, L\}$, and at the base
level $|\Atom_6| = |\Obs_6| = 3$ (the level-$6$ obstructions
$\{19, 27, 59\}$ are atomic by Remark~\ref{rem:empty-below-six}). For
$L \ge 7$, Theorem~\ref{thm:no-G0-atom} kills all $G_0$-atoms above the
base level, so
\[
|\Obs_L| = |\Atom_6| \cdot 2^{L-6} + \sum_{L_0 = 7}^{L} |\Atom_{L_0}^{G_{\ne 0}}| \cdot 2^{L - L_0}
...
$\Obs_L \leftrightarrow \Obs_L^{+}$. At the base level $L = 6$ this
identifies $\Obs_6^{+} = \{2^6 - r : r \in \Obs_6\} = \{5, 37, 45\}$,
the explicit anchor set used in the $\Tplus$ analogue of the
...
\bottomrule
\end{tabular}
\caption{Direct enumeration counts at levels $L=6,\dots,16$: total obstructions, shift-zero and shift-nonzero subclasses, and atomic shift-nonzero obstructions. The decomposition $|\Obs_L| = |\Obs_L^{G_0}| + |\Obs_L^{G_{\ne 0}}|$ is read off each row, and the strict lift balance $|\Obs_L| = 2|\Obs_{L-1}| + |\Atom_L^{G_{\ne 0}}|$ (Corollary~\ref{cor:lift-balance}) holds row-by-row against the previous one.}
