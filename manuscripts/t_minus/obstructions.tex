...
statement is vacuous, and the proof in Section~\ref{sec:lift} only uses
$L_0 \ge 2$.) The lift is explicit: for each $r_0$, the terminal data of the
...
An odd integer $r$ with $v := \vtwo(r-1) \ge 1$ is an
\emph{obstruction residue at level $L > v$} if $\Xend(r, L) = 1$. We write
\[
\Obs_L := \{\, r \in \{1, \ldots, 2^L\} : r \text{ is an obstruction residue at level } L \,\}.
\]
The typical case is $v = 1$ (equivalently $r \equiv 3 \pmod 4$); larger
values of $v$ contribute additional residues whose analysis is identical
...
\begin{proof}
Set $v := \vtwo(r_0 - 1) \ge 1$; the typical case is $v = 1$.
...
$\Vk^{(J)} + \vk^{(J)} \ge L$. We split by whether this inequality holds
at index $J$ or already at index $J - 1$ (i.e.\ $\Vk^{(J-1)} + \vk^{(J-1)} \ge L - 1$,
which is the corresponding stop condition at level $L - 1$); the two cases
are exhaustive because if the level-$(L-1)$ stop condition failed at every
$j \le J - 1$, the reduction at $(r', L-1)$ would not terminate by index $J$,
contradicting $r' \in \{1, \ldots, 2^{L-1}\}$.
