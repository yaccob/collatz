...
| `obstr_factor_complexity.py` | Theorem 5.1 (full factor complexity): empirical enumeration of $p_W(n)$ at finite $L$. For $n$ close to or exceeding $L$ the displayed value is below $2^n$ - a finite-$L$ artefact, not a refutation; see `factor_complexity_construction.py` for the constructive complement. |
...
| `iso_synchronisation_verification.py` | Corollary 2.5 (simultaneous stop): K- and I-track terminate at the same index. Verified for all $G_0$-obstructions at $L \le 16$. |
