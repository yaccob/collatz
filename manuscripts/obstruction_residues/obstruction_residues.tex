...
$5+1, \ldots, 5+\ell$ --- reading positions $6, \ldots, 5+\ell$ in increasing
order spells out $u_0 u_1 \cdots u_{\ell-1} = u$. Thus
...
$\Vk^{(J)} = \Vk^{(J-1)} + \vk^{(J-1)}$ also satisfies $\Vk^{(J)} < L$, since
the level-$L$ stop does not fire at $J - 1$. With $\Vk^{(J)} \ge L - 1$ this
forces the only integer value $\Vk^{(J)} = L - 1$. Hence the
...
of $J$. \emph{Stop at $J$:} the inductive step at $j = J - 1$ requires both reductions
to run only at $J - 1$, not at $J$, so it applies and yields the residue
congruence~\eqref{eq:invol-residue} at $j = J$.
