...
and we set $q := \Vi^{(J)} - J$. Since the I-track fires $J$ steps each of
valuation $\ge 1$ (\cref{cor:sync}), $\Vi^{(J)} \ge J$, so $q \ge 0$.
...
are attained by geometric extremal sequences (all I-excess in the first step;
the K-excess deferred to the final, uncounted step), giving the closed forms
\[
