...
on the I-track (note the extra $-v$): on the K-track
...
one: $r = 7$ shares with every obstruction an \emph{even} terminal valuation
difference,
\[
\Vk^{(J)} - \Vi^{(J)} - v = 2 - 3 - 1 = -2,
\]
...
\]
satisfy $\Delta_K^{(0)} = 2^{L_0}$ and $\Delta_I^{(0)} = 2^{L_0 - v}$. Here
$\Delta_I^{(0)} = (r_- - 1)/2^v - m_0 = 2^{L_0}/2^v$, using
$\vtwo(r_- - 1) = \min(\vtwo(r_0 - 1), L_0) = v$ --- the last equality because
$L_0 > v$. Throughout this proof a superscript $-$ on a step valuation
(e.g.\ $\vk^{(J),-}$ below) marks that valuation in the $(r_-, L_0+1)$ reduction.
...
$\vtwo(\Delta_I^{(J)}) = L_0 - v - \Vi^{(J)} < \vi^{(J)}$. Substituting the
...
stop conditions at $j = J$ therefore read
\[
\Vk^{(J)} + \vk^{(J)} \ge L .
\]
...
$L - 1$ fires at $J$ and the reduction terminates there. By the same chain the
I-track stop also fires at $J$ (the synchronisation $\vi^{(J)} = \vk^{(J)}$,
$\Vk^{(J)} - \Vi^{(J)} = v$ holds at $j = J$); for termination only the first
track to stop matters. The
...
here $\Vi^{(J)} = J + q$ has been substituted into the terminal exponent. The two sums range over the structural families: by
...
monotonicity allow, putting all I-excess in the first step. Dually,
$A_K^{\min}$ keeps the K-valuations on their lower envelope, with the single
forced jump of \cref{lem:k-prefix} deferred to the final, uncounted step. Both extremal sums
...
For $s \ge 3$ and $1 \le J \le 2s+2$ one has $\Gamma(J,s) > 0$, since the sign
change $2^{J}(4^{s}+2) = 2\cdot 3^{J}$ occurs only at
$J \approx s\log_{3/2}4 > 2s+2$, beyond the stated range. Hence $D$ is
...
of $J$. \emph{Stop at $J$:} we apply the inductive step one final time to obtain the
residue congruence~\eqref{eq:invol-residue} at $j = J$. This is legitimate: the
inductive hypothesis at $j = J - 1$ requires only that both reductions run at
$J - 1$, not at $J$. Combining this congruence with the
$\Tminus$ K-track stop
