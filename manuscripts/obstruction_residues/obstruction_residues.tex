...
extremised at the envelope of its constraints. Thus $A_I$ is maximised by
making the I-valuations as large as the endpoint $\Vi^{(J)} = J+q$ and strict
monotonicity allow (all I-excess in the first step), and $A_K$ is minimised by
keeping the K-valuations on their lower envelope with the single forced jump of
\cref{lem:k-prefix} deferred to the final, uncounted step. (Both extremal sums
are also checked against a brute optimisation over the families in
...
$C_0(J,v) = 2^{v} 3^{J}\bigl(3^{1-v}-1\bigr) - 6\cdot 2^{J}$.
For $s \ge 3$ and $1 \le J \le 2s+2$ one has $\Gamma(J,s) > 0$ (the sign change
$2^{J}(4^{s}+2) = 2\cdot 3^{J}$ occurs at $J \approx s\log_{3/2}4 > 2s+2$), so $D$ is
...
each strictly increasing valuation contributes one set bit, the carry is bounded
(each transition halves it towards a fixed invariant interval), and an obstruction
with parameter $q$ corresponds exactly to an accepting run (carry $0$, all
valuations placed). The positions factor as $\text{prefix} \to \text{block } A
...
$J = 6$, on which $\Gamma(6,2) < 0$ makes $q$ unbounded. The stratum $(s=2, J=6)$
is closed for all $q$ by \cref{lem:carry-automaton}; the finitely many residual
cells are closed by exhaustive search over the coupled valuation sequences. All
