...
valuation of its modulus --- $L - \Vk^{(j)}$ on the K-track, $(L - v) - \Vi^{(j)}$
on the I-track (note the extra $-v$):
\[
...
one: $r = 7$ already has the \emph{even} terminal valuation difference
\[
\Vk^{(J)} - \Vi^{(J)} - v = 2 - 3 - 1 = -2
\]
shared by every obstruction. Yet it fails $\Xend = 1$, its terminal $d_J$
coming out wrong: by the parity lemma below the even difference is necessary
for every obstruction, but $r = 7$ shows it is not sufficient.
\end{example}
...
\]
using $\vtwo(r_- - 1) = \min(\vtwo(r_0 - 1), L_0) = v$, the last equality
because $L_0 > v$.
...
\]
so the ultrametric identity gives
...
The parallel reduction at $(r', L-1)$ --- the level-$(L-1)$ companion of the
level-$L$ reduction at $(r, L)$, with the same two tracks $K, I$ --- starts from
...
$\Vk^{(J'+1)} \ge L - 1$. By the no-further-step fact (Section~\ref{sec:setup})
the $(r, L)$ reduction then stops at $J' + 1 \le J - 1$, contradicting the
definition of $J$. The same bound on the I-track,
$\Vi^{(J'+1)} \ge L - 1 - v$, rules out a child I-stop at $J' \le J - 2$ by the
same fact. This proves the claim.

At index $J - 1$, a child I-stop requires $\Vi^{(J)} \ge L - 1 - v$. By the
terminal synchronisation $\Vi^{(J)} = \Vk^{(J)} - v$ (from $s = 0$),
\[
\Vi^{(J)} \ge L - 1 - v \quad\Longleftrightarrow\quad \Vk^{(J)} \ge L - 1,
\]
the Case~B branch below; so in Case~A neither child stop fires before index
$J$.
...
extremised at the envelope of its constraints. Concretely, $A_I^{\max}$ puts all I-excess in the first step (I-valuations as
large as the endpoint $\Vi^{(J)} = J+q$ and strict monotonicity allow), while
$A_K^{\min}$ keeps the K-valuations on their lower envelope, deferring the
single forced jump of \cref{lem:k-prefix} to the final, uncounted step. Both extremal sums
...
$s\log_{3/2}4 > 2s+2$ for $s \ge 3$ places it past the window. Hence $D$ is
...
$L - v - \Vi^{(j)}$ and the K-track numerators $3 a_K^{\pm,(j)} \pm 1$ by the
I-track numerators $3 a_I^{\pm,(j)} \pm 1$ --- gives the analogous
