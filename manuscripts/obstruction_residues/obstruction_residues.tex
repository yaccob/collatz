...
termination. In particular, once a track's cumulative valuation reaches
$\Vk^{(j)} \ge L - 1$ on the K-track (or $\Vi^{(j)} \ge L - v - 1$ on the
I-track), no further step is possible there: the residual modulus valuation is
then $\le 1$, while the relevant $a^{(j)}$ is odd, so $3 a^{(j)} - 1$ is even,
the step valuation is $\ge 1$, and the stop condition is met. We invoke this
\emph{no-further-step fact} wherever a lift or a child reduction adds at most
one further step.
...
$X_j := 3 d_j + c_j$,
\begin{align*}
A_I^{(j)} &= 3(c_j a_K^{(j)} + d_j) - 1 \\
   &= c_j (3 a_K^{(j)} - 1) + (3 d_j + c_j - 1) \\
   &= c_j A_K^{(j)} + (X_j - 1).
\end{align*}
...
\end{array}
\]
Both tracks stop at $j = 1$, each step valuation reaching the residual
valuation of its modulus --- $L - \Vk^{(j)}$ on the K-track, and
$(L - v) - \Vi^{(j)}$ on the I-track, which carries the extra $-v$:
\[
\vk^{(1)} = 1 \ge L - \Vk^{(1)} = 5 - 4 = 1, \qquad
\vi^{(1)} = 3 \ge (L - v) - \Vi^{(1)} = 4 - 1 = 3.
\]
The terminal pair is
...
\]
so $7 \notin \Obs_5$. The pair separates a necessary condition from a sufficient
one: $r = 7$ already has the \emph{even} terminal valuation difference $\delta_J$
of \cref{lem:parity} (below). Its terminal track totals are $\Vk^{(J)} = 2$,
$\Vi^{(J)} = 3$ (with $v = 1$), so
\[
\delta_J = \Vk^{(J)} - \Vi^{(J)} - v = 2 - 3 - 1 = -2,
\]
the even difference shared by every obstruction. Yet it fails $\Xend = 1$: the
even difference is necessary for an obstruction but, as $r = 7$ shows, not
sufficient.
\end{example}
...
Combining with the residual valuations from Section~\ref{sec:setup},
$\vtwo(b_I^{(J)}) = (L-v) - \Vi^{(J)}$ and $\vtwo(b_K^{(J)}) = L - \Vk^{(J)}$,
and using $\Vk - \Vi = 2s + v$:
\begin{align*}
\vi^{(J)} \ge \vtwo(b_I^{(J)})
...
Taking one more step, with $c_J = 4^s$, $d_J = (1 - 4^s)/3$, and
$\vk^{(J)} - \vi^{(J)} = -2s$ by the synchronisation of \cref{cor:sync}:
\[
...
\]
After this step, $\Vk^{(J+1)} = L_0$, so the K-track residual modulus has
valuation $1$; by the no-further-step fact (Section~\ref{sec:setup}) no further
step is possible. Termination
occurs at index $J+1$ with
$(c_{J+1}, d_{J+1}) = (1, 0)$, hence $\Xend = 1$ and
$r_+ \in \Obs_{L_0+1}^{G_0}$.

\emph{Stop type SS at $L_0$.} Here $\vk^{(J)} > L_0 - \Vk^{(J)}$. Both
sides are integers, so $\vk^{(J)} \ge L_0 + 1 - \Vk^{(J)}$, which is exactly the
...
those at $(r_-, L_0+1)$; a superscript $-$ on a step valuation (e.g.\
$\vk^{(J),-}$ below) likewise marks that valuation in the $(r_-, L_0+1)$
reduction. The discrepancies
\[
\Delta_K^{(j)} := a_{K,-}^{(j)} - a_{K,+}^{(j)}, \qquad \Delta_I^{(j)} := a_{I,-}^{(j)} - a_{I,+}^{(j)}
\]
satisfy $\Delta_K^{(0)} = 2^{L_0}$ and $\Delta_I^{(0)} = 2^{L_0 - v}$. Since
$L_0 > v$, $\vtwo(r_- - 1) = \min(\vtwo(r_0 - 1), L_0) = v$, so the $r_-$
I-track uses the same shift exponent $v$, and
\[
...
The argument at every index rests on the same decomposition. Writing
$A_{K,-}^{(j)} := 3 a_{K,-}^{(j)} - 1$ and $A_{I,-}^{(j)} := 3 a_{I,-}^{(j)} - 1$,
whenever \eqref{eq:lift-minus-delta} holds at step $j$ we have
$\vtwo(3\Delta_K^{(j)}) = L_0 - \Vk^{(j)}$ and
\[
A_{K,-}^{(j)} = (3 a_{K,+}^{(j)} - 1) + 3 \Delta_K^{(j)},
\]
so, comparing the step valuation $\vk^{(j)}$ with the residual-modulus
valuation $L_0 - \Vk^{(j)}$, the ultrametric identity gives
\begin{align*}
\vtwo(A_{K,-}^{(j)}) &= \min\bigl(\vk^{(j)},\, L_0 - \Vk^{(j)}\bigr)
  &&\text{if } \vk^{(j)} \ne L_0 - \Vk^{(j)},\\
\vtwo(A_{K,-}^{(j)}) &\ge \vk^{(j)} + 1
  &&\text{if } \vk^{(j)} = L_0 - \Vk^{(j)}
\end{align*}
(and analogously on the I-track, with $L_0 - v - \Vi^{(j)}$ in place of
$L_0 - \Vk^{(j)}$).
...
$\vk^{(j)} < L_0 - \Vk^{(j)}$, so the decomposition gives
$\vtwo(A_{K,-}^{(j)}) = \vk^{(j)}$, proving \eqref{eq:lift-minus-vmatch} at
step $j$. The basic step then yields
$\Delta_K^{(j+1)} = (3 \Delta_K^{(j)})/2^{\vk^{(j)}} = 2^{L_0 - \Vk^{(j+1)}} \cdot 3^{j+1}$
(using $\Vk^{(j+1)} = \Vk^{(j)} + \vk^{(j)}$); the same calculation on the
I-track completes \eqref{eq:lift-minus-delta} at step $j+1$.

\emph{Stop type SS at $L_0$.} Throughout this case, unmarked $\vk^{(J)},
\vi^{(J)}$ denote the $(r_0, L_0)$ step valuations and the marked
$\vk^{(J),-}, \vi^{(J),-}$ the $(r_-, L_0+1)$ ones. At index $J$ of
$(r_+, L_0+1)$, $\vk^{(J)} > L_0 - \Vk^{(J)}$. The decomposition therefore gives
$\vk^{(J),-} := \vtwo(A_{K,-}^{(J)}) = L_0 - \Vk^{(J)} < L_0 + 1 - \Vk^{(J)}$,
so the K-step at $(r_-, L_0+1)$ at index $J$ is permitted. On the I-track the
decomposition likewise gives $\vi^{(J),-} := L_0 - v - \Vi^{(J)}$. Permitting
the I-step additionally requires the strict inequality
$\vtwo(\Delta_I^{(J)}) = L_0 - v - \Vi^{(J)} < \vi^{(J)}$, which we now verify.
Substituting the
synchronisation $\vi^{(J)} = \vk^{(J)} + 2s$ of \cref{cor:sync} and
$\Vk^{(J)} - \Vi^{(J)} = 2s + v$ turns this into
\[
L_0 - v - \Vi^{(J)} < \vi^{(J)}
\quad\Longleftrightarrow\quad
\vk^{(J)} > L_0 - \Vk^{(J)},
\]
the K-track inequality already in force. The new exponent in the
update rule is $\vk^{(J),-} - \vi^{(J),-} = -2s$, so the extra step closes
exactly as in the proof of \cref{thm:lift-plus}:
\[
(c_{J+1}, d_{J+1}) = (1, 0).
\]
After it $\Vk^{(J+1)} = L_0$ and
...
\emph{Stop type EE at $L_0$.} Here $\vk^{(J)} = L_0 - \Vk^{(J)}$, so the
decomposition's equal-valuation branch gives
$\vtwo(A_{K,-}^{(J)}) \ge \vk^{(J)} + 1 = L_0 + 1 - \Vk^{(J)}$, which is the
K-track stop condition at level $L_0+1$. The same argument on the I-track,
verbatim with $L_0 - v - \Vi^{(J)}$ for $L_0 - \Vk^{(J)}$, gives
...
\[
|\Obs_{L_0+1}| \ge 2 |\Obs_{L_0}|,
\qquad\text{equivalently}\qquad
\frac{|\Obs_{L_0+1}|}{2^{L_0+1}} \ge \frac{|\Obs_{L_0}|}{2^{L_0}},
\]
so the density ratio is non-decreasing. Since
$\Obs_L \subseteq \{1, \ldots, 2^L\}$ the ratio is bounded above by $1$. By
...
\]
(The new bit sits at position $5+k$, the top bit of an element of $\Obs_{6+k}$.)
We claim $r_k \in \Obs_{6+k}$ for each $k$. The base $k = 0$ is by choice of
$r_0$. For the inductive step from $k-1$ to $k$, split on the bit $u_{k-1}$.
If $u_{k-1} = 0$ then $r_k = r_{k-1}$ is the $r_+$ lift, so
$r_k \in \Obs_{6+k}$ by \cref{thm:lift-plus}. If $u_{k-1} = 1$ then
$r_k = r_{k-1} + 2^{5+k}$ is the $r_-$ lift, so $r_k \in \Obs_{6+k}$ by
\cref{thm:lift-minus}.
...
\]
so $u$ appears as a factor of $w_{r_\ell}$: reading bit positions
$6, \ldots, 5+\ell$ in increasing order spells out
$u_0 u_1 \cdots u_{\ell-1} = u$. Thus
...
(iii) Primitive substitution shifts have factor complexity in one of the
polynomial classes
\[
\Theta(1),\quad \Theta(n),\quad \Theta(n \log \log n),\quad \Theta(n \log n),\quad \Theta(n^2)
\]
(Pansiot~\cite{Pansiot1984}; refined by Devyatov~\cite{Devyatov2015}), and
therefore have topological entropy zero. But the obstruction family has
$p_W(\ell) = 2^\ell$, hence entropy $\log 2 > 0$; so it cannot be the factor
language of a primitive substitution shift. \qedhere
\end{proof}
...
$k = 0, 1, \ldots, L - 5$ --- as $k$ increases the level $L-k$ decreases, so
larger $k$ gives a coarser residue. By the lift theorem (\cref{thm:lift-intro}),
whenever $r_{k+1} \in \Obs_{L-k-1}$, the level-$(L-k)$ residue
$r_k = r \bmod 2^{L-k}$ is one of the two lifts of $r_{k+1}$, hence
lies in $\Obs_{L-k}$. The converse need not hold. Let
$L_0$ be the smallest level $\ge 5$ for which
$r \bmod 2^{L_0} \in \Obs_{L_0}$. We check that
$r \bmod 2^{L_0 - 1} \notin \Obs_{L_0 - 1}$. For $L_0 > 5$ this is the
minimality of $L_0$; for $L_0 = 5$ it follows from $\Obs_4 = \emptyset$
(\cref{rem:smallest-level}). Hence
...
Recall that $\vk^{(j)}, \vi^{(j)}$ are the per-step valuations and
$\Vk^{(j)}, \Vi^{(j)}$ their running sums on the two tracks.
...
\]
whose numerator $-(1 + 2^{1-v})$ is strictly negative, so $d_1 \ne 0$, again
contradicting $d_J = 0$. Hence $J \ge 2$, so the case
distinction below over the indices $J - 1$ and $J$ is well-defined.
The parallel reduction at $(r', L-1)$ is the level-$(L-1)$ companion of the
level-$L$ reduction at $(r, L)$, with the same two tracks $K, I$: it starts from
the same residue class $a_K \bmod 2^{L-1}, a_I \bmod 2^{L-1-v}$, with the
modulus reduced by one power of two. As in the lift-minus calculation of
\cref{thm:lift-minus}, the two K-tracks (resp.\ I-tracks) differ as integers by
$\Delta_K^{(j)} = 2^{L-1-\Vk^{(j)}} \cdot 3^j$ (resp.\
$\Delta_I^{(j)} = 2^{L-1-v-\Vi^{(j)}} \cdot 3^j$). Whenever
$\vk^{(j)} < L - 1 - \Vk^{(j)}$, the ultrametric identity forces
$v_2(3 a_K^{(j)} - 1)$ to agree at $(r, L)$ and $(r', L-1)$, and symmetrically
$v_2(3 a_I^{(j)} - 1)$ agrees whenever $\vi^{(j)} < L - 1 - v - \Vi^{(j)}$.
Consequently the
...
\vi^{(J)} \ge (L - v) - \Vi^{(J)}
\quad\Longleftrightarrow\quad
\vk^{(J)} \ge (L - v) - (\Vk^{(J)} - v) = L - \Vk^{(J)} .
\]
Both stop conditions at $j = J$ therefore read
\[
\Vk^{(J)} + \vk^{(J)} \ge L .
\]
This equivalence holds only at the terminal index $J$: at $j < J$ the K- and
I-track stop conditions differ, so the child stops below must be ruled out on
...
\emph{Claim: no level-$(L-1)$ stop fires on either track at an index
$J' \le J - 2$.} Suppose the K-track stop fired there, so
$\Vk^{(J')} + \vk^{(J')} \ge L - 1$. The $(r, L)$ reduction still takes its
$J'$-th step, since the level-$L$ stop has not yet fired at $J' < J$, so
\[
\Vk^{(J'+1)} \ge L - 1, \qquad\text{i.e.}\qquad L - \Vk^{(J'+1)} \le 1 .
\]
By the no-further-step fact (Section~\ref{sec:setup}) the $(r, L)$ reduction
then stops at $J' + 1 \le J - 1$, contradicting the definition of $J$. The same bound on the I-track,
$\Vi^{(J'+1)} \ge L - 1 - v$, rules out a child I-stop at $J' \le J - 2$ by the
same fact. This proves the claim.

\emph{Case A: $\Vk^{(J)} < L - 1$.} No level-$(L-1)$ child stop fires before
index $J$ on either track: the indices $J' \le J - 2$ are ruled out by the
Claim, and at $J' = J - 1$ a child stop would force $\Vk^{(J)} \ge L - 1$ ---
directly on the K-track, since $\Vk^{(J)} = \Vk^{(J-1)} + \vk^{(J-1)}$, and on
the I-track via the terminal synchronisation $\Vi^{(J)} = \Vk^{(J)} - v$ (from
$s = 0$),
\[
\Vi^{(J)} \ge L - 1 - v \quad\Longleftrightarrow\quad \Vk^{(J)} \ge L - 1,
\]
against the Case-A hypothesis $\Vk^{(J)} < L - 1$. So the $(r', L-1)$ reduction
runs at least as far as index $J$; at $j = J$,
\[
\Vk^{(J)} + \vk^{(J)} \ge L > L - 1,
\]
hence the K-track stop at level $L - 1$ fires at $J$ and the reduction
terminates there. By the terminal
synchronisation already noted the I-track stop also fires at $J$; for
termination only the first track to stop matters. The
...
$\Vk^{(J-1)} + \vk^{(J-1)} \ge L - 1$.} The quantity
$\Vk^{(J)} = \Vk^{(J-1)} + \vk^{(J-1)}$ satisfies $\Vk^{(J)} < L$, since
the level-$L$ stop does not fire at $J - 1$. With the Case-B hypothesis
$\Vk^{(J)} \ge L - 1$,
\[
L - 1 \le \Vk^{(J)} < L,
\]
so the only integer value is $\Vk^{(J)} = L - 1$. Hence the
...
$3 d_{J-1} + c_{J-1} - 1 = 0$, i.e.\ the $X$-invariant
$X_{J-1} = 3 d_{J-1} + c_{J-1}$ equals $1$. By the parity lemma
...
\emph{Notation.} Throughout this subsection $s = s(r, L)$ is the (signed) shift
index of an obstruction $r \in \Obs_L$, $J = J(r, L)$ its termination index,
$v = \vtwo(r-1)$, and
\[
q := \Vi^{(J)} - J .
\]
Since the I-track takes $J$ steps, each of valuation $\ge 1$ (as
$3 a_I^{(j)} - 1$ is even for odd $a_I^{(j)}$), $\Vi^{(J)} \ge J$, so $q \ge 0$.
...
\begin{proof}
If $J \le v-1$, then $\Vk^{(J)} = J$ by \cref{lem:k-prefix}. But also
$\Vk^{(J)} = \Vi^{(J)} + 2s + v \ge J + v$, using $\Vi^{(J)} \ge J$ and
$s \ge 0$. Combined with $\Vk^{(J)} = J$ this forces $v \le 0$, a
contradiction.
\qedhere
\end{proof}
...
\]
Unrolling the recurrence from $\tilde n_0 = -1$,
\[
3\,\tilde n_J = -3^{J+1} + \sum_{j=0}^{J-1} 3^{\,J-j}\bigl(2^{\Vk^{(j)}} - 2^{\Vi^{(j)}+v}\bigr),
\]
while the shift-$s$ normal form gives
$3 \tilde n_J = 2^{\Vi^{(J)}+v} \cdot 3 d_J = 2^{\Vi^{(J)}+v}(1 - 4^{s})$; equating
the two yields~\eqref{eq:oform}. Since
$\Vk^{(J)} = \Vi^{(J)} + v + 2s$, the right-hand side of~\eqref{eq:oform} equals
\[
3^{J+1} + 2^{\Vi^{(J)}+v} - 2^{\Vk^{(J)}} .
\]
Moving the two terminal terms into the respective sums --- extending each from
$j = J-1$ to $j = J$ --- turns~\eqref{eq:oform} into~\eqref{eq:signfree}.
\qedhere
\end{proof}
...
\hat A_K \;\le\; 2^{\Vk^{(J)}} \sum_{i=0}^{J} (3/2)^{i}
   \;=\; 2^{\Vk^{(J)}}\Bigl(\frac{3^{J+1}}{2^{J}} - 2\Bigr)
...
\begin{proof}
Recall $q = \Vi^{(J)} - J \ge 0$, $v = \vtwo(r-1)$, and $s \ge 4$ here.
By \cref{lem:depth-min}, $v \le J$. Write the O''-form~\eqref{eq:oform} as
$A_K - 2^{v} A_I = 3^{J+1} + 2^{\,J+q+v}(1 - 4^{s})$, where
$A_K = \sum_{j=0}^{J-1} 3^{J-j} 2^{\Vk^{(j)}}$ and $A_I$ is the analogous I-sum;
here $\Vi^{(J)} = J + q$ has been substituted into the terminal exponent. The
two sums range over the structural families ($v$ bounds the forced K-prefix,
$q$ the terminal I-excess): by
...
extremised at the envelope of its constraints. Concretely, $A_I^{\max}$ takes the I-valuations as large as the endpoint
$\Vi^{(J)} = J+q$ and strict monotonicity allow, i.e.\ all I-excess in the first
step, while $A_K^{\min}$ keeps the K-valuations on their lower envelope,
deferring the single forced jump of \cref{lem:k-prefix} to the final, uncounted
step. Both extremal sums
are independently confirmed by a brute optimisation over the families in
Appendix~\ref{sec:appendix}. This gives the closed forms (with $A_K, A_I$
running to $j = J-1$)
\[
...
\]
and, substituting both into $D$ (the $3^{J+1}$ in $A_K^{\min}$ cancelling the
$-3^{J+1}$ in $D$),
\[
...
For $s \ge 3$ and $1 \le J \le 2s+2$ one has $\Gamma(J,s) > 0$: the sign
change $2^{J}(4^{s}+2) = 2\cdot 3^{J}$ lies at $J \approx s\log_{3/2}4$, and
$s\log_{3/2}4 > 2s+2$ for $s \ge 3$ (equivalently $\log_{3/2}4 > 2 + 2/s$)
places it past the window. Since
$D = C_0 + 2^{v+q}\Gamma$ with $\Gamma > 0$, $D$ is non-decreasing in $q$; so
$q = 0$ is the worst case. At $q = 0$,
$D(J,s,v,0) = 2^{v}\,\Phi(J,s) + R(J,v)$, where the $q=0$ envelope
$\Phi(J,s) = 2^{J}(4^{s}+2) - 3^{J+1} = \Gamma(J,s) - 3^{J}$ and the
$v$-correction $R(J,v) = 2^{v} 3^{\,J-v+1} - 6\cdot 2^{J}$.
For $s \ge 4$ and $J \le 2s+2$, $\Phi(J,s) > 3$ (worst case $J = 2s+2$:
$\Phi(2s+2,s) > 3 \iff (16/9)^{s} > 27/4$, which holds for $s \ge 4$). Finally
$R(J,v) = 3\cdot 2^{v}\bigl(3^{\,J-v} - 2^{\,J-v+1}\bigr)$. When $J - v \ge 2$,
$R \ge 0$, so $D(J,s,v,0) = 2^{v}\Phi + R \ge 2^{v}\Phi > 0$. When
$J - v \in \{0,1\}$, $R = -3\cdot 2^{v}$, so
$D(J,s,v,0) = 2^{v}(\Phi - 3) > 0$ since $\Phi > 3$. Thus $D > 0$ for all $q \ge 0$ and all
...
Each strictly increasing valuation contributes one set bit. The carry stays
...
Let $r$ be the $\Tminus$ residue, odd with $v := \vtwo(r-1) \ge 1$ (the
$\Tminus$ convention, not the $\Tplus$ one $\vtwo(r+1)$) and $L > v$, and set
$\overline{r} := 2^L - r$ for its $\Tplus$ partner. In the conclusions below, write
$\Vk^{(j)} := \sum_{i < j} \vk^{(i)}$ and
...
\begin{proof}
Throughout, a superscript $+$ or $-$ marks the $\Tplus$ or $\Tminus$ reduction
and the subscript $K$ or $I$ the two tracks; $\vk, \vi$ are per-step valuations
and $\Vk, \Vi$ their running sums. The two reductions are tied throughout by the
sign flip $a_K^{+,(j)} \equiv -a_K^{-,(j)}$ of~\eqref{eq:invol-residue} (the
residue-level shadow of $\overline{r} = 2^L - r$). The I-track modulus carries
the extra $-v$
(its residual valuation is $L - v - \Vi^{(j)}$, against $L - \Vk^{(j)}$ on the
K-track) because the I-track starts at level $L - v$.
...
way. The induction hypothesis at index $j$ is that \eqref{eq:invol-residue}
holds and $\vk^{+,(i)} = \vk^{-,(i)}$, $\vi^{+,(i)} = \vi^{-,(i)}$ for all
$i < j$.

\emph{Base case ($j = 0$).} On the two tracks,
\[
a_K^{+,(0)} = \overline{r} = 2^L - r = -a_K^{-,(0)} + 2^L, \qquad
a_I^{+,(0)} = (\overline{r}+1)/2^v = 2^{L-v} - a_I^{-,(0)},
\]
using $a_I^{-,(0)} = (r-1)/2^v$ and $\overline{r}+1 = 2^L - (r-1)$. Both
congruences in~\eqref{eq:invol-residue} hold at $j = 0$. For the
valuation equality at $j = 0$, suppose the $\Tminus$ reduction is still
running, i.e.\ $\vk^{-,(0)} < L$. Then
$3 \overline{r} + 1 = 3 \cdot 2^L - (3 r - 1)$, whose summands have
valuations $L$ and $\vk^{-,(0)}$. Since $\vk^{-,(0)} < L$,
...
that both reductions are still running at index $j$. By the inductive
valuation equalities $\vk^{+,(j)} = \vk^{-,(j)}$ and
$\vi^{+,(j)} = \vi^{-,(j)}$ we may write $\vk^{(j)}, \vi^{(j)}$ without the
$\pm$ superscript. In this notation the running condition reads
$\vk^{(j)} < L - \Vk^{(j)}$ and $\vi^{(j)} < L - v - \Vi^{(j)}$. For the
K-track,
\[
...
\pmod{2^{L - \Vk^{(j+1)}}}.
\]
The same calculation on the I-track --- replacing $L - \Vk^{(j)}$ by
$L - v - \Vi^{(j)}$ and the K-track numerators $3 a_K^{+,(j)} + 1$,
$3 a_K^{-,(j)} - 1$ by the I-track numerators $3 a_I^{+,(j)} + 1$,
$3 a_I^{-,(j)} - 1$ --- gives the analogous congruence and
$\vi^{+,(j)} = \vi^{(j)}$.
...
stop:} suppose the $\Tplus$ reduction stopped at some $j' < J$. Both
reductions still run at $j'$ (the $\Tminus$ one because $j' < J$), so the
inductive step applies and gives $\vk^{+,(j')} = \vk^{-,(j')}$ and
$\vi^{+,(j')} = \vi^{-,(j')}$. But then the $\Tminus$ reduction would also
satisfy its stop condition at $j'$, contradicting the definition
of $J$. \emph{Stop at $J$:} the inductive step at $j = J - 1$ needs both
reductions to run at $J - 1$ (its hypothesis), which holds; it therefore
applies and yields the residue congruence~\eqref{eq:invol-residue} at $j = J$.
Combining this congruence with the
$\Tminus$ K-track stop
...
terminal index $J$, i.e.\ $(c_J^{+}, d_J^{+}) = (c_J^{-}, -d_J^{-})$, together
with the sign convention $\Xend^{+} := c - 3d$ of \cref{rem:Xplus-sign} gives
\[
...
$r_0 := 2^{L_0} - \overline{r_0} \in \Obs_{L_0}$, so that $\overline{r_0} =
2^{L_0} - r_0$ is the \emph{level-$L_0$} involution image of $r_0$; by
\cref{thm:lift-intro}, both $r_0$ and $r_0 + 2^{L_0}$ lie in $\Obs_{L_0+1}$.
Their level-$(L_0+1)$ involution images, re-expressed through that same
$\overline{r_0}$,
\[
