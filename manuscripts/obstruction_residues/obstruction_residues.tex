...
Let $L_0 \ge 5$ and $r_0 \in \Obs_{L_0}$. Then both $r_+ := r_0$ and
...
finite invariant of the parallel reduction. (The hypothesis $L_0 \ge 5$
matches the smallest level for which $\Obs_{L_0}$ is non-empty, namely
$L_0 = 5$ with $\Obs_5 = \{27\}$ (Remark~\ref{rem:smallest-level}); for
$L_0 < 5$ the statement is vacuous. The proof in Section~\ref{sec:lift}
only uses $L_0 > v$, which is automatic from $r_0 \in \Obs_{L_0}$ by
...
\delta_J := \Vk^{(J)} - \Vi^{(J)} - v
\]
is an \emph{even} integer (possibly negative) with $|\delta_J| \le L$,
where $v := \vtwo(r-1)$, and for any index $j$ we write
...
In both cases $\delta$ is even. For the bound, $\Vk^{(J)} \in [0, L]$ and
$\Vi^{(J)} \in [0, L - v]$ (cumulative valuations are bounded by the
respective initial modulus exponents), so
$\Vk^{(J)} - \Vi^{(J)} \in [v - L, L]$ and hence
$\delta_J = \Vk^{(J)} - \Vi^{(J)} - v \in [-L, L - v] \subseteq [-L, L]$.
\qedhere
\end{proof}
...
By the bound $\delta_J \in [-L, L - v]$ from Lemma~\ref{lem:parity},
$a = \delta_J/2 \in [-L/2,\, (L - v)/2] \subseteq [-L/2,\, L/2]$;
in particular $|a| \le L/2$.
...
$L_0 \ge 5$. Specialising to $L_0 = 6$ and using the direct enumeration
...
The conclusion also holds at $L = 5, 6$ by direct enumeration (see the
proof of Theorem~\ref{thm:series}); the bound $L \ge 7$ in the statement
is the proof-method threshold, because the case analysis below requires
$J \ge 2$, which is established as the first step of the proof.
...
anchor at some level $L_0 \in \{5, 6, \ldots, L\}$. Direct enumeration
at the base levels (Remark~\ref{rem:smallest-level}; verified by
\texttt{count\_obstructions.py} at $L = 5, 6$) gives
$\Atom_5 = \{27\}$ and $\Atom_6 = \{19\}$, both with shift $a = 1$, so
$|\Atom_5^{G_0}| = |\Atom_6^{G_0}| = 0$ and
$|\Atom_5^{G_{\ne 0}}| = |\Atom_6^{G_{\ne 0}}| = 1$; the remaining
level-$6$ obstructions $\{27, 59\} \subset \Obs_6$ are the two lifts of
$27 \in \Atom_5$ and are not atomic at $L = 6$. For $L \ge 7$,
Theorem~\ref{thm:no-G0-atom} kills all $G_0$-atoms above the base
levels, so
\[
