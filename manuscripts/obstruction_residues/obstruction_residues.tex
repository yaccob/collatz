...
% Examples carry their own counter (like figures/tables) so that adding one
% never renumbers the theorem-counter results referenced by the supporting
% code and knowledge-graph instance.
...
lockstep (Corollary~\ref{cor:sync}) and collapses their terminal affine
relation to the shift-$s$ normal form developed below; a generic residue
exhibits no such coupling. The name also has a dynamical reading: $\Xend = 1$
implies a synchronization of stopping times --- starting from $r$ and from any
...
paper develops only the algebraic side; the dynamical interpretation
motivates the definition but is logically independent of the proofs below.
...
constitute Theorem~\ref{thm:lift-intro}. Throughout, $s \in \Z$ is the
shift index of $r_0$ at level $L_0$, and we use the synchronization
$\vi^{(J)} = \vk^{(J)} + 2s$ (Corollary~\ref{cor:sync}) freely.
...
$\vi^{(J)} = \vk^{(J)} + 2s$ of Corollary~\ref{cor:sync} together with
...
\end{tikzpicture}
\caption{Two generations of lifts (Proposition~\ref{prop:lift-symmetry}). A
...
the strict lift balance of Corollary~\ref{cor:lift-balance}.}
\label{fig:lift-tree}
\end{figure}
...
Corollary~\ref{cor:entropy} in fact excludes the entire zero-entropy
...
the parity lemma (Lemma~\ref{lem:parity}) yields the shift-$s$ normal
...
outside that classification (Remark~\ref{rem:non-primitive}). The natural
