...
one: $r = 7$ already has the \emph{even} terminal valuation difference (the
quantity $\delta_J$ of \cref{lem:parity} below)
\[
\Vk^{(J)} - \Vi^{(J)} - v = 2 - 3 - 1 = -2
\]
shared by every obstruction. Yet it fails $\Xend = 1$: the even difference is
necessary for an obstruction but, as $r = 7$ shows, not sufficient.
\end{example}
...
$\vtwo(A_{K,-}^{(j)}) = \min(\vk^{(j)}, L_0 - \Vk^{(j)})$ when
$\vk^{(j)} \ne L_0 - \Vk^{(j)}$, and $\vtwo(A_{K,-}^{(j)}) \ge \vk^{(j)} + 1$
when the two are equal (and analogously on the I-track, with
$L_0 - v - \Vi^{(j)}$ in place of $L_0 - \Vk^{(j)}$).
...
$\vk^{(J)} > L_0 - \Vk^{(J)}$. The decomposition therefore gives
$\vk^{(J),-} := \vtwo(A_{K,-}^{(J)}) = L_0 - \Vk^{(J)} < L_0 + 1 - \Vk^{(J)}$,
so the K-step at $(r_-, L_0+1)$ at index $J$ is permitted (the unmarked
$\vk^{(J)}, \vi^{(J)}$ are the $(r_0, L_0)$ values, the marked
$\vk^{(J),-}, \vi^{(J),-}$ the $(r_-, L_0+1)$ ones). On the I-track the
...
K-track stop condition at level $L_0+1$. The same argument on the I-track gives
...
zero. But the obstruction family has $p_W(\ell) = 2^\ell$, hence entropy
...
on the I-track when $\vi^{(j)} < L - 1 - v - \Vi^{(j)}$). Consequently the
...
and $\Vk, \Vi$ their running sums. The I-track modulus carries the extra $-v$
...
$\vi^{+,(j)} = \vi^{-,(j)}$ we may write $\vk^{(j)}, \vi^{(j)}$ unambiguously,
and the running condition reads $\vk^{(j)} < L - \Vk^{(j)}$ and
$\vi^{(j)} < L - v - \Vi^{(j)}$. For the
K-track,
\[
...
of $J$. \emph{Stop at $J$:} applying the inductive step at $j = J - 1$ ---
legitimate, since that step needs both reductions to run only at $J - 1$, not
at $J$ --- yields the residue congruence~\eqref{eq:invol-residue} at $j = J$.
