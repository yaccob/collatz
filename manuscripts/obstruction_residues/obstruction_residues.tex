...
exhibits no such coupling. The name also has a dynamical reading: $\Xend = 1$ is precisely the
condition under which the two parallel reductions stop simultaneously
(\cref{cor:sync}), a rigidity uniform over the residue class
$n \equiv r \pmod{2^L}$. The present paper develops only this algebraic
side; the dynamical interpretation in terms of $\Tminus$-trajectory
stopping times motivates the definition but is logically independent of
the proofs below.
...
base cases to \cref{cor:no-G0-atom-all} for clarity.
