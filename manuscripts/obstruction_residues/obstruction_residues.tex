...
through $\{17, 25, 37, 55, 65, 49, 73, \ldots\}$; see
...
the set of all such residues. Definition~\ref{def:obstruction} in
...
(Remark~\ref{rem:smallest-level}) and the statement is vacuous; the
smallest non-vacuous level is $L_0 = 5$ with $\Obs_5 = \{27\}$. The
proof in Section~\ref{sec:lift} only uses $L_0 > v$, which is automatic
from $r_0 \in \Obs_{L_0}$ by Definition~\ref{def:obstruction}.) The lift is explicit: for each $r_0$,
...
(Section~\ref{sec:Ga}) then re-expresses $c_W$ as a convergent series and
...
of $r$ at level $L$, denoted $a(r, L)$. In particular,
$\Vk^{(J)} - \Vi^{(J)} = 2a + v$. The obstruction family partitions:
\[
\Obs_L = \bigsqcup_{a \in \Z} \Obs_L^{G_a}, \qquad \Obs_L^{G_a} := \{\, r \in \Obs_L : a(r, L) = a \,\}.
\]
For $a = 0$ we have $(c_J, d_J) = (1, 0)$, meaning $a_I^{(J)} = a_K^{(J)}$.
By the bound $\delta_J \in [-(L-1), L - 1 - v]$ from Lemma~\ref{lem:parity},
$a = \delta_J/2 \in [-(L-1)/2,\, (L - 1 - v)/2]$; in particular
$|a| \le (L-1)/2$.
...
terminal step valuations satisfy $\vi^{(J)} = \vk^{(J)} + 2a$, where $a$
is the shift index of $r$ at level $L$ from the shift-$a$ normal form above.
\end{corollary}
...
$\vi^{(J)} > \vtwo(b_I^{(J)})$. By Corollary~\ref{cor:sync} these two
...
of $r_0$ at $L_0$ (Definition~\ref{def:stop-type}) as follows (both lifts
at level $L_0+1$):
...
For every $n \ge 0$, $p_W(n) = 2^n$.
\end{theorem}

\begin{proof}
The case $n = 0$ is the empty word, which is trivially a factor of every
$w_r$, so $p_W(0) = 1 = 2^0$. Fix $n \ge 1$ and
$u = (u_0, u_1, \ldots, u_{n-1}) \in \{0,1\}^n$. Choose
...
zero. The obstruction family has $p_W(n) = 2^n$ and hence entropy
...
Corollary~\ref{cor:no-substitution}(iii) is the scope of the
Pansiot--Devyatov classification we invoke; whether $\Fac(\Obs)$ can be
realised as the factor language of a non-primitive substitution shift
remains open (see Section~\ref{sec:open}).
\end{remark}
...
$k = 0, 1, \ldots, L - 5$. By the lift theorem (Theorem~\ref{thm:lift-intro}),
...
and $\Obs_4 = \emptyset$ by Remark~\ref{rem:smallest-level}, or by
...
The conclusion also holds at $L = 5, 6$ by direct enumeration (see
Corollary~\ref{cor:no-G0-atom-all}); the bound $L \ge 7$ in the statement
is conservative. The case analysis below applies verbatim at $L = 6$
and is vacuous at $L = 5$, where $|\Obs_5^{G_0}| = 0$; we defer both
base cases to Corollary~\ref{cor:no-G0-atom-all} for clarity.
...
the lift-minus calculation of Theorem~\ref{thm:lift-minus}. Whenever
...
do. Since $a = 0$, at the terminal index $J$ the synchronisation
$\vi^{(J)} = \vk^{(J)} + 2a = \vk^{(J)}$ and
$\Vk^{(J)} - \Vi^{(J)} = v$ holds (Lemma~\ref{lem:parity}), so the
...
(At intermediate indices $j < J$ the synchronisation does not hold in
general, so the K- and I-track conditions are not in general equivalent
there; the case analysis below is stated in K-track terms only and does
not rely on equivalence at $j < J$.)
...
$a = 0$, equivalently $\delta_J = 0$), this is exactly
...
rule (Lemma~\ref{lem:affine}) at index $J - 1$,
\[
...
\begin{proof}
For $L \ge 7$, this is Theorem~\ref{thm:no-G0-atom}. At the two base
...
Corollary~\ref{cor:no-G0-atom-all} occur only in $h_L$:
...
\begin{proof}
By Lemma~\ref{lem:atom-decomp} every $r \in \Obs_L$ has a unique atomic
anchor at some level $L_0 \in \{5, 6, \ldots, L\}$. By
Corollary~\ref{cor:no-G0-atom-all}, $|\Atom_{L_0}^{G_0}| = 0$ for every
...
$\Atom_6 = \{19\}$, both with shift index $a = 1$, so
...
\]
By Theorem~\ref{thm:density-intro}, $\rho_L$ is non-decreasing in $L$ and
...
$\overline{r} := 2^L - r$. Then $\vtwo(\overline{r}+1) = v$ as well
(part~(i) of the lemma), so the $\Tplus$ reduction at $\overline{r}$ in
the sense of Section~\ref{sec:tplus} uses the same parameter $v$. In
the conclusions below, $\Vk^{(j)} := \sum_{i < j} \vk^{(i)}$ and
$\Vi^{(j)} := \sum_{i < j} \vi^{(i)}$ denote the common cumulative
valuations of the two reductions, equal across $\Tplus/\Tminus$ by the
valuation equalities established in the proof. The conclusions:
\begin{enumerate}[label=(\roman*)]
...
\eqref{eq:invol-affine} of Lemma~\ref{lem:involution-step}(iv) at the
terminal index $J$ together with the sign convention
$\Xend^{+} := c - 3d$ of Remark~\ref{rem:Xplus-sign} gives
\[
...
by Corollary~\ref{cor:no-substitution}, so any realisation must lie
outside that classification (Remark~\ref{rem:non-primitive}).
...
Corollary~\ref{cor:lift-balance} --- is a natural and apparently open
...
Significant partial results are also known for general multiplier $a$ ---
in particular structure theorems for $a = -1$ and for the
negative-Mersenne family $a = -(2^{v} - 1)$ with explicit lower bounds
on $c_W$. A complete treatment of the $(ax+b)$ family raises new
...
Theorem~\ref{thm:density-intro} is conditional on the correctness of this
enumeration. The check is self-validating in a useful sense: the
strict lift balance of Corollary~\ref{cor:lift-balance} is asserted
