...
- not a refutation of full factor complexity (`thm:fc-intro`), just a finite-L artefact. The constructive
...
def main():
    ...
    if len(ns_fit) >= 2:
        ...
        # Full factor complexity (`thm:fc-intro`) predicts p_W(n) = 2^n once L is large enough that the
        ...
        if abs(slope - math.log(2)) < 0.01:
            print("\n✓ VERIFIED: empirical slope matches log 2 within 0.01 (`thm:fc-intro`).")
        else:
            print(
                f"\n  Note: empirical slope {slope:.4f} at L={L_use} is below log 2 = {math.log(2):.4f};"
                f"\n  this is expected at finite L (truncation of the language). For rigorous"
                f"\n  verification of p_W(n) = 2^n, see factor_complexity_construction.py."
            )
